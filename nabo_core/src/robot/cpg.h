...
namespace Cpg
{
	...
	public:
		...
		const double &getSwT(int i) { return swT[i]; };  // get摆动时间
		const double &getStT(int i) { return stT[i]; };  // get摆动时间
		const double getSwTicRm(int i) { return swT[i] - swTic2[i]; }; // 剩余摆动时长
		const double getStTicRm(int i) { return stT[i] - stTic[i]; };  // 剩余站立时长
	private:
		cpgClass();
		...
		double dt, period;
		double offset[Num], duty[Num];  // ~[0,1] // duty取0.5 0.5， 两个脚对称
		struct
		{
