...
namespace Crv
{
	...
	public:
		swTrj6dClass() {}
		void setRP0(const mat3d &RR, const vec3d &pp)
		{
			R0 = RR;
			p0 = pp;
		}
		...
		void setRP1(const mat3d &RR, const vec3d &pp)
		{
			R1 = RR;
			p1 = pp;
		}														   // 一锤定音
		...
		void getRP0(mat3d &RR, vec3d &pp)
		{
			RR = R0;
			pp = p0;
		}
		void getRP1(mat3d &RR, vec3d &pp)
		{
			RR = R1;
			pp = p1;
		}
	...
	//==swing trajectory== 点足用的3d的曲线
	class swTrj3dClass
	{
	public:
		swTrj3dClass() {}
		void setP0(const vec3d &pp) { p0 = pp; }
		void setP0now() { p0 = p; }
		void setP1(const vec3d &pp) { p1 = pp; } // 一锤定音
		void setP1(const vec3d &pp, double pgs); // 有权重，适合实时set
		void setH(const double h) { this->h = h; }
		...
		void getP(vec3d &pp) { pp = p; }
		void getV(vec3d &vv) { vv = v; }
		void getP0(vec3d &pp) { pp = p0; }
		void getP1(vec3d &pp) { pp = p1; }
